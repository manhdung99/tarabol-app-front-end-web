...
export default function ChangePhoneModal({
  ...
  const [phoneCurrent, setPhoneCurrent] = useState(currentUser.phone);
  ...
  const handleUpdatePhone = () => {
    if (currentPass === currentUser.password) {
      let newCurrentUser = { ...currentUser, phone: phoneCurrent };
      let newUsers = [...users];
      ...
      <div ref={phoneRef} className="w-[440px] bg-[#36393f] rounded-[8px]">
        <div className="px-6 py-4 text-center relative">
          ...
          <p className="text-[#B9BBBE]">
            Enter a new phone number and your existing password
          </p>
          <span
